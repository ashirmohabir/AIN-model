...
def main():
    ...
    np.savetxt("nb_AIN_confusion_matrix.txt", conf_matrix, fmt='%d', delimiter=',')
    ...
    results.to_csv("nb_AIN_predictions.csv", index=False)

    print(classification_report(y_test, y_pred, zero_division=0))

    with open('nb_AIN_classification_report.txt', 'w') as f:
    ...
    # Save the DataFrame to a CSV file
    report_df.to_csv('nb_AIN_classification_report.csv')
